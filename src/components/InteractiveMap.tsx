...
import { County, CountyFeature, CountyFeatureState, CountyObject, getFeatureIdentifier, natureOptions } from '../utils/utils'
...
export const InteractiveMap: FC<InteractiveMapProps> = forwardRef((props, ref) => {
  ...
  const renderCounties = (counties: CountyObject) => {
  ...
  return (
    <Map
      ...
      onLoad={(e) => renderCounties(counties)}
    >
      {children}
    </Map>
  )
